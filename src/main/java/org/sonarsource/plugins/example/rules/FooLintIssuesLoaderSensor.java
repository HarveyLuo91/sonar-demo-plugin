...
//        System.out.println("------------------config farther:" + config.getClass().getSuperclass());
////    if(config instanceof DefaultConfiguration){
////      System.out.println("------------------DefaultConfiguration properties:" + ((DefaultConfiguration)config).getProperties().toString());
////    }
////    if(config instanceof ModuleSettings){
////      System.out.println("------------------ModuleSettings properties:" + ((ModuleSettings)config).getProperties().toString());
////    }
//        Method[] methods = config.getClass().getSuperclass().getDeclaredMethods();
//        for (Method method : methods) {
//            if (method.getName().startsWith("get")) {
//                try {
//                    System.out.println("--------------------------- method name:" + method.getName() + "  value:" + method.invoke(config));
//                } catch (Exception e) {
//                }
//            }
//        }
    ...
    public void execute(final SensorContext context) {
        ...
        if (reportPath != null) {
            ...
            try {
                System.out.println("-----------------------start parseAndSaveResults method");
    ...
    protected void parseAndSaveResults(final File file) throws XMLStreamException {
        ...
        System.out.println("-------------------------(mock) Parsing 'FooLint' Analysis Results");
    ...
    private void getResourceAndSaveIssue(final ErrorDataFromExternalLinter error) {
        ...
        System.out.println("inputFile null ? " + (inputFile == null));
        if (inputFile != null) {
            saveIssue(inputFile, error.getLine(), error.getType(), error.getDescription());
        } else {
            System.out.println("Not able to find a InputFile with " + error.getFilePath());
